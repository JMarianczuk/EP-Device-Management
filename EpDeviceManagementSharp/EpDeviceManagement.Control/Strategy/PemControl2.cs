...
public class PemControl2 : IEpDeviceController
{
        ...
        bool withGeneration)
    {
        ...
        if (targetRelativeSoC < probabilisticModeLowerLevel || targetRelativeSoC > probabilisticModeUpperLevel)
        {
            throw new ArgumentOutOfRangeException(nameof(targetRelativeSoC), targetRelativeSoC,
                "cannot be out of the bounds");
        }

        this.probabilisticModeLowerLevel = probabilisticModeLowerLevel;
        this.probabilisticModeUpperLevel = probabilisticModeUpperLevel;
        this.targetRelativeSoC = targetRelativeSoC;
        ...
        this.targetSoC = battery.TotalCapacity * targetRelativeSoC.DecimalFractions;
        this.targetFactor = (targetRelativeSoC - probabilisticModeLowerLevel) /
                            (probabilisticModeUpperLevel - targetRelativeSoC);
    ...
    public string Name => "PEM Control 2";

    public string Configuration => string.Create(CultureInfo.InvariantCulture,
        $"[{this.probabilisticModeLowerLevel.DecimalFractions:F1}, {this.probabilisticModeUpperLevel.DecimalFractions:F1}] ({this.targetRelativeSoC.DecimalFractions:F1})");
