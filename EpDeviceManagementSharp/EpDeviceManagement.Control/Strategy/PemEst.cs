...
public class EstimationPemControl : IEpDeviceController
{
    ...
    {
        this.assumedSoC = LinearProbabilisticEstimationRangeControl.CalculateAssumedCurrentBatterySoC(
            timeStep,
            load,
            generator,
            this.Battery.CurrentStateOfCharge);
        if (this.assumedSoC >= probabilisticModeUpperLimit)
        {
            stateMachine.Fire(Event.BatteryAboveSetpoint);
        }
        else if (this.assumedSoC <= probabilisticModeLowerLimit)
        {
    ...
    public string Name => "PEM Control + EST";
